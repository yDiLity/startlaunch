import { AlertCircle, AlertTriangle, Shield, XCircle } from "lucide-react";
				...
				<div className="flex items-start space-x-3">
					...
					<div>
						...
						<p className="text-green-300 text-sm">
							Этот репозиторий находится в списке доверенных. Проект будет
							запущен без дополнительных проверок.
						</p>
					</div>
				</div>
			</div>
			...
			<div className={`border rounded-lg p-4 ${getBgColor(warnings[0].level)}`}>
				<div className="flex items-start justify-between mb-3">
					<div className="flex items-start space-x-3">
						...
						<div>
							<h4
								className={`font-semibold ${getTextColor(warnings[0].level)} mb-1`}
							>
								{hasCriticalWarnings
				...
				<div className="space-y-3">
					{warnings.map((warning) => (
						<div key={warning.id} className="bg-gray-800/50 rounded p-3">
				...
				{hasCriticalWarnings && (
					<div className="mt-4 p-3 bg-red-950/50 border border-red-800 rounded">
						<p className="text-red-300 text-sm font-semibold">
							⚠️ НЕ ЗАПУСКАЙТЕ ЭТОТ ПРОЕКТ! Обнаружены критические угрозы
							безопасности, которые могут повредить вашу систему.
						</p>
					</div>
				)}
				...
				{!hasCriticalWarnings && onTrustRepository && !isTrusted && (
					<div className="mt-4 flex items-center justify-between p-3 bg-gray-800/50 rounded">
						<div className="flex-1">
							<p className="text-gray-300 text-sm">
								Вы доверяете этому репозиторию? Добавьте его в список
								доверенных, чтобы пропустить проверки безопасности в будущем.
							</p>
						</div>
						<button
							type="button"
