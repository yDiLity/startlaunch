  ...
  const handleBrowseSnapshotsPath = async () => {
    try {
      ...
      if (selected && typeof selected === 'string') {
        setSettings(prev => prev ? { ...prev, snapshots_path: selected } : null);
      }
    } catch (err) {
          ...
          <div className="setting-item">
            ...
            <select
              ...
              onChange={(e) => setSettings({
                ...settings,
                default_isolation_mode: e.target.value as 'Sandbox' | 'Direct'
              })}
            >
              <option value="Sandbox">Песочница (Docker) - Рекомендуется</option>
              <option value="Direct">Прямой режим (Виртуальное окружение)</option>
            </select>
          </div>
        </div>
        ...
        <div className="settings-section">
          ...
          <div className="setting-item">
            <label htmlFor="max-age">
              <span className="setting-label">Максимальный возраст снимков (дни)</span>
              <span className="setting-description">
            ...
            <input
              ...
              onChange={(e) => setSettings({
                ...settings,
                max_snapshot_age_days: parseInt(e.target.value) || 30
              })}
            />
          </div>
        </div>
