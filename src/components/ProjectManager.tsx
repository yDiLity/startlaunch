...
import { Search, Tag, Play, Trash2, Calendar, Folder, X } from "lucide-react";
...
const ProjectManager: React.FC<ProjectManagerProps> = ({ onClose, onLaunchProject }) => {
  ...
  const loadProjects = async () => {
    try {
      ...
      setError(err.user_friendly_message || "Ошибка загрузки проектов");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteProject = async (projectId: string) => {
    ...
    try {
      ...
      setError(err.user_friendly_message || "Ошибка удаления проекта");
    }
  };
  ...
  const handleLaunchProject = async (projectId: string) => {
    try {
      ...
      setError(err.user_friendly_message || "Ошибка запуска проекта");
    }
  };
